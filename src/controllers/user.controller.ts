...
import { UserIdDto, UserWechatLoginDto, UserWechatGetPhoneDto } from '@dtos/user.dto'
...
export class UserController {
  ...
  async mockLogin(@Query() query: UserIdDto, @Res({ passthrough: true }) response: Response): Promise<UserInfoVo> {
  ...
  async info(@Req() request: Request): Promise<UserInfoVo> {
    return await this.userService.info(request.user.id)
  }
}
