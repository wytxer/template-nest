...
export class UserService {
  ...
  async wechatLogin(body: UserWechatLoginDto): Promise<UserInfoVo> {
    ...
    return await this.userRepo
      ...
      .then(async (user) => {
        ...
        // 更新用户信息
        if (user) {
          await user.update({ openId, loggedAt })
          const { id, nickname, avatarUrl, phone } = user
          return { id, nickname, avatarUrl, phone }
        }
        // 新增用户
        return await this.userRepo.create({ openId, unionId, loggedAt }).then(({ id, nickname, avatarUrl, phone }) => {
          return { id, nickname, avatarUrl, phone }
        })
      })
  }
   ...
   * @param id
   */
  async info(id: string): Promise<UserInfoVo> {
