...
import { IS_TEST, IS_LOCAL } from '@helpers/env.helper'
...
async function bootstrap() {
  ...
  // 接口文档配置
  if (IS_TEST || IS_LOCAL) {
    const options = new DocumentBuilder().setTitle('接口文档标题').setDescription('接口文档描述').build()
  ...
  await app.listen(port, config.get('server.hostname'), async () => {
    logger.log(`[template-nestjs-standalone] 服务启动成功：${await app.getUrl()}`)
  })
}
bootstrap()
